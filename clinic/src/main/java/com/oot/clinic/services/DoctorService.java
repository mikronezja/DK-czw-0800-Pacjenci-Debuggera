...
public class DoctorService {
    ...
    public Doctor addDoctor(Doctor doctor) {
        // Trim whitespace
        if (doctor.getName() != null) doctor.setName(doctor.getName().trim());
        if (doctor.getSurname() != null) doctor.setSurname(doctor.getSurname().trim());
        ...
        if (doctor.getName() != null && doctor.getName().length() > 100) {
            throw new RuntimeException("Name is too long (max 100 characters)");
        }
        if (doctor.getSurname() != null && doctor.getSurname().length() > 100) {
        ...
        if (doctor.getPesel() != null && doctor.getPesel().length() > 11) {
